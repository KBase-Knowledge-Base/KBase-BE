...
| Folder / Category / Tag | A | M9 suites vẫn pass; M11 API test chứng minh folder/category/tag cross-project bị reject lúc upload; M13 contract test verify role docs trong spec | Cao - product/design/service/API rules rõ | Authorization, hierarchy, uniqueness, delete dependency và cross-project integrity đã được test | Chi tiết field-level trong api-schema.md vẫn sync thủ công | 2026-09-18 |
| Document / MinIO / Search | A | M12 service 2/2, controller regression 1/1, search API 2/2 và JPA repository 11/11 pass trên PostgreSQL Testcontainers; full suite 209/209 | Cao | Storage/lifecycle/search authorization boundary và project isolation được cover; tag `EXISTS` avoids duplicate rows; multipart/binary/Range docs được verify bởi contract test | Chi tiết field-level trong api-schema.md vẫn sync thủ công | 2026-09-18 |

## Lớp Kiến trúc

| Lớp | Điểm | Thực thi Ranh giới | Khả năng đọc của Agent | Khoảng trống chính | Cập nhật lần cuối |
|-------|-------|---------------------|-----------------|----------|-------------|
| Controller / API | A | M13 OpenAPI runtime: springdoc spec từ controllers/DTO với security requirements, role rules, multipart/binary/Range docs và 21 contract tests chặn drift | Cao - REST/OpenAPI design có sẵn và runtime spec đã verify | Swagger UI chưa smoke qua browser thật (MockMvc đủ cho contract) | 2026-09-18 |
| Service / Authorization | A | `ProjectAuthorizationService` được tái sử dụng cho M12 read access; mandatory project predicate, canonical sort whitelist và metadata-only query đã verify | Cao - Service/Security design rõ | Full Docker runtime thuộc M14 | 2026-09-18 |
...
|---|---|---|---|---|
| Backend | A | M0–M13 gồm OpenAPI runtime springdoc với security/permission/multipart/binary documentation; full regression 209/209 | M14 full Docker runtime | 2026-09-18 |
...
| API contract | A | Runtime OpenAPI `/v3/api-docs` từ springdoc 3.1.1 là contract máy đọc được; `OpenApiContractIntegrationTest` 19/19 + `OpenApiDisabledIntegrationTest` 2/2 verify security scheme, requirements, multipart/binary/range schemas, error codes, sensitive-field absence; `docs/generated/api-schema.md` đồng bộ theo runtime | Markdown snapshot sync thủ công; chưa có snapshot/breaking-change CI diff (optional theo SD-11) | 2026-09-18 |
...
| Kiểm thử | A | M13 OpenAPI contract 21/21 (enabled + disabled), M12 search suite và mọi regression trước đó; full suite 209/209 | Swagger UI smoke qua browser thật thuộc runtime verification (M14) | 2026-09-18 |

...
| AI Chatbot / RAG | C | M1 foundation + M2 persistence + M3 durable lifecycle đã verify: typed disabled-by-default config, KBase-owned ports, deterministic fakes, Flyway V4/pgvector/HNSW, SQL project isolation, FK/delete/status guards, `SKIP LOCKED` job engine, bounded scheduler, document intent/delete races và membership retention; full M3 gate 268/268 | Cao ở mức persistence/async lifecycle/source docs; application behavior boundary còn rõ | Provider/RAG/chat application vẫn chưa có | Chưa có Gemini provider call/adapter, extraction/indexing execution, retrieval/grounding, conversation/Guide runtime hoặc AI API; real Gemini smoke intentionally absent | 2026-09-22 |
...
|---|---|---|---|---|
| Backend | A | M0–M16 Core implementation/maintenance + M1–M3 AI foundation/lifecycle verified; Core v1 frozen; M3 full regression 268/268 | Core ổn định; provider/RAG application là phase tiếp theo và chưa thay đổi Core API | 2026-09-22 |
| AI / RAG | C | M0 decision log + M1/M2 executable evidence + M3 job/lifecycle evidence: typed config, vendor-neutral ports, deterministic fakes, pgvector V4 schema, SQL isolation, FK/status guards, `SKIP LOCKED`, lease/retry/dedup, document intent/delete safety và retention hooks | Chưa có Gemini provider adapter/call, extraction/indexing execution, retrieval/grounding, conversation/Guide runtime hoặc AI API; real Gemini chưa gọi | 2026-09-22 |
...
| Tích hợp hệ thống | A | Core Docker runtime smoke + M1/M2/M3 Testcontainers: PostgreSQL 17.11 pgvector, V1–V4 fresh/upgrade, vector SQL isolation, durable job claim/lease, document/retention races, Redis/MinIO wiring, full regression 268/268; mail double cho automated path | Gmail thật manual smoke chưa chạy; AI provider/runtime chat journeys chưa bắt đầu | 2026-09-22 |
| Kiểm thử | A | M1 pgvector compatibility 1/1, M2 targeted 36/36, M3 targeted 27/27, integrity 13/13, AI persistence 11/11, upgrade 1/1, full `mvn -B -ntp clean verify` 268/268; Core golden journeys vẫn giữ pass | Docker smoke chưa script hóa (đã ghi command trong DEVELOPMENT.md) | 2026-09-22 |
| Thiết lập development | A | `docker compose` config + Testcontainers pgvector 0.8.6/PostgreSQL 17.11 verified; V4 fresh/upgrade, M3 job/lifecycle commands, reset/start commands và generated DB schema tài liệu hóa; AI disabled mặc định | Full Compose runtime với AI V4 chưa re-run trong slice này; Gmail thật chưa verify | 2026-09-22 |
| Reliability | A | Core golden journeys trong Docker runtime (M14/M15) và M3 durable job/document/retention race suites trên PostgreSQL đã pass; lease-token/stale recovery, delete no-resurrection, rejoin cancel và bounded scheduler invariants được ghi trong RELIABILITY | Health/metrics endpoint chuyên dụng chưa có (không yêu cầu Core v1); provider/runtime chat monitoring là scope sau | 2026-09-22 |
| Deployment và rollback | A | M14/M15 Dockerfile + Compose/runtime verification; M1 giữ healthcheck-gated topology, named volumes và pgvector-compatible image; M2 additive V4 migration/rollback policy và Compose config pass | AI V4 clean Compose runtime chưa re-run; chưa có deployment target production (chưa khóa; không tự chọn K8s/Terraform) | 2026-09-22 |

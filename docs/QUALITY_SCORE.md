...
| AI Chatbot / RAG | C | M1 foundation đã verify: Spring AI 2.0.1/Google GenAI starters + pgvector JDBC resolve, typed config disabled-by-default, KBase-owned ports, deterministic fakes, pgvector PostgreSQL 17.11 compatibility 1/1 và full suite 228/228 | Cao ở mức M0/M1 source docs và plan; behavior boundary còn rõ | Foundation tests ổn định; RAG behavior chưa có | Chưa có provider adapter/call, AI schema, worker, indexing, retrieval, conversation/Guide behavior hoặc AI API; real Gemini smoke intentionally absent | 2026-09-22 |
...
|---|---|---|---|---|
| Backend | A | M0–M16 Core implementation/maintenance verified; Core v1 frozen; M1 AI foundation full regression 228/228 | Core ổn định; AI v1 là phase riêng và chưa thay đổi Core API/schema | 2026-09-22 |
| AI / RAG | C | M0 decision log + M1 executable evidence: typed config, vendor-neutral provider ports, deterministic fakes, disabled startup và pgvector compatibility | Chưa có provider adapter, AI schema/jobs/conversation/Guide/RAG runtime; M2–M11 chưa implement; real Gemini chưa gọi | 2026-09-22 |
...
| Tích hợp hệ thống | A | Core Docker runtime smoke + M1 Testcontainers: PostgreSQL 17.11 pgvector, Redis/MinIO wiring, full regression 228/228; mail double cho automated path | Gmail thật manual smoke chưa chạy; M2 AI schema chưa bắt đầu | 2026-09-22 |
| Kiểm thử | A | M1 pgvector compatibility 1/1, AI targeted 15/15, full `mvn -B -ntp clean verify` 228/228; Core golden journeys vẫn giữ pass | Docker smoke chưa script hóa (đã ghi command trong DEVELOPMENT.md) | 2026-09-22 |
| Thiết lập development | A | `docker compose` config + Testcontainers pgvector 0.8.6/PostgreSQL 17.11 verified; reset/start commands tài liệu hóa; AI disabled mặc định | Full Compose runtime với M1 image chưa re-run trong slice này; Gmail thật chưa verify | 2026-09-22 |
...
| Deployment và rollback | A | M14/M15 Dockerfile + Compose/runtime verification; M1 giữ healthcheck-gated topology, named volumes và đổi PostgreSQL default sang pgvector-compatible image; Compose config pass | Chưa có deployment target production (chưa khóa; không tự chọn K8s/Terraform) | 2026-09-22 |

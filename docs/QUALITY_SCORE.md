...
| AI Chatbot / RAG | B+ | M0–M10 evidence plus M11 isolated Docker V1–V4 startup, Project Assistant `READY`/grounded/no-evidence and Guide grounded/no-evidence through real HTTP; safe provider-unavailable 503 while Core remains healthy | Cao ở application/REST boundary, corpus isolation, guard contract và source docs | Security, retention and restart/recovery Docker evidence remain; real Gemini smoke absent; M5 worker lease heartbeat còn là debt | 2026-09-25 |
...
|---|---|---|---|---|
| Backend | A | M0–M16 Core implementation/maintenance + M1–M10 AI foundation, indexing, strict RAG, private REST, retention and usage/contract hardening verified; Core v1 frozen; M11 Docker startup and initial HTTP journeys pass | M11 security, retention, restart/recovery and final freeze evidence remain | 2026-09-25 |
| AI / RAG | B+ | M6 real pgvector trap/grounding, M7 PostgreSQL/JWT conversation lifecycle, M8 retention, M9 Guide allowlist and M10 Redis guard/threshold/observability/OpenAPI hardening; M11 deterministic Docker provider path and initial journeys pass | Restart/freeze evidence, security/retention runtime matrix remain unavailable | 2026-09-25 |
...
| Kiểm thử | A | M10 suites, M11 deterministic configuration tests and real Docker AI initial journeys; Core regression retained | Full M11 runtime matrix still needs executable evidence | 2026-09-25 |
| Thiết lập development | A | `docker compose` base + mail-double config và Testcontainers pgvector 0.8.6/PostgreSQL 17.11/Redis 7.4 verified; V4 fresh/upgrade, M3 lifecycle, M4 provider, M5 extraction/indexing, M10 guard/OpenAPI commands và generated DB schema được tài liệu hóa | Full AI-enabled Compose startup/indexing chưa verify; Docker runtime provider injection path chưa được approved; Gmail thật chưa verify | 2026-09-25 |
...
| Deployment và rollback | A | M14/M15 Dockerfile + Compose/runtime verification; M1 healthcheck topology, M2 additive V4 policy, M4 disabled-by-default provider configuration/request-timeout, M5 no-topology/no-migration change, M10 usage/threshold env passthrough and M11 Compose config checks pass | AI-enabled deployment/connectivity chưa verify vì deterministic Docker provider path thiếu; connect-timeout SDK limitation, M5 lease limitation and chat recovery debt remain tracked | 2026-09-25 |

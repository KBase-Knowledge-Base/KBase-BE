...
| AI Chatbot / RAG | C | M1 foundation + M2 persistence đã verify: Spring AI 2.0.1/Google GenAI starters + pgvector JDBC resolve, typed config disabled-by-default, KBase-owned ports, deterministic fakes, Flyway V4 với 8 AI tables, pgvector/HNSW, JPA repositories, SQL project isolation, FK/delete/status guards, quota lock và full gate 241/241 | Cao ở mức persistence/source docs; application behavior boundary còn rõ | M2 schema và DB guards ổn định; provider/RAG behavior chưa có | Chưa có provider adapter/call, worker, extraction/indexing, retrieval/grounding, conversation/Guide behavior hoặc AI API; real Gemini smoke intentionally absent | 2026-09-22 |
...
|---|---|---|---|---|
| Backend | A | M0–M16 Core implementation/maintenance verified; Core v1 frozen; M1 foundation + M2 AI persistence full regression 241/241 | Core ổn định; AI v1 behavior là phase riêng và chưa thay đổi Core API | 2026-09-22 |
| AI / RAG | C | M0 decision log + M1/M2 executable evidence: typed config, vendor-neutral provider ports, deterministic fakes, pgvector V4 schema, HNSW, SQL project isolation, FK/delete/status guards và concurrency lock | Chưa có provider adapter, worker, extraction/indexing, retrieval/grounding, conversation/Guide runtime hoặc AI API; real Gemini chưa gọi | 2026-09-22 |
| Frontend | - | `docs/FRONTEND.md` | Optional và hoãn khỏi phase hiện tại | 2026-09-17 |
| Database và migration | A | Flyway V1–V4 fresh apply + Core V1–V3 upgrade trên pgvector PostgreSQL 17.11; integrity 13/13, AI persistence 11/11, mapping 11/11, upgrade 1/1; Hibernate validate; `docs/generated/db-schema.md` đã đối chiếu 18 tables, vector(768), HNSW/FK/index catalog | Chưa có generator schema tự động; full Compose runtime với AI V4 chưa re-run trong slice này | 2026-09-22 |
...
| Tích hợp hệ thống | A | Core Docker runtime smoke + M1/M2 Testcontainers: PostgreSQL 17.11 pgvector, V1–V4 fresh/upgrade, vector SQL isolation, Redis/MinIO wiring, full regression 241/241; mail double cho automated path | Gmail thật manual smoke chưa chạy; AI provider/runtime journeys chưa bắt đầu | 2026-09-22 |
| Kiểm thử | A | M1 pgvector compatibility 1/1, M2 targeted 36/36, integrity 13/13, AI persistence 11/11, upgrade 1/1, full `mvn -B -ntp clean verify` 241/241; Core golden journeys vẫn giữ pass | Docker smoke chưa script hóa (đã ghi command trong DEVELOPMENT.md) | 2026-09-22 |
| Thiết lập development | A | `docker compose` config + Testcontainers pgvector 0.8.6/PostgreSQL 17.11 verified; V4 fresh/upgrade commands, reset/start commands và generated DB schema tài liệu hóa; AI disabled mặc định | Full Compose runtime với AI V4 chưa re-run trong slice này; Gmail thật chưa verify | 2026-09-22 |

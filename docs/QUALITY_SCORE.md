...
| AI Chatbot / RAG | C | M1 foundation + M2 persistence + M3 durable lifecycle + M4 provider boundary đã verify: typed disabled-by-default config, KBase-owned ports, deterministic fakes, Flyway V4/pgvector/HNSW, SQL project isolation, durable jobs, explicit Gemini/Spring AI adapters, strict 768 vectors, safe error categories và privacy guards; full M4 gate 290/290 | Cao ở mức persistence/async/provider boundary/source docs; application behavior boundary còn rõ | Extraction/indexing, retrieval/grounding, conversation/Guide runtime và AI API vẫn chưa có; real Gemini smoke intentionally absent | 2026-09-22 |
...
|---|---|---|---|---|
| Backend | A | M0–M16 Core implementation/maintenance + M1–M4 AI foundation/lifecycle/provider boundary verified; Core v1 frozen; M4 full regression 290/290 | Core ổn định; extraction/indexing/RAG application là phase tiếp theo và chưa thay đổi Core API | 2026-09-22 |
| AI / RAG | C | M0 decision log + M1/M2 executable evidence + M3 job/lifecycle + M4 adapter evidence: typed config, vendor-neutral ports, deterministic fakes, pgvector V4 schema, SQL isolation, durable lease/retry/dedup, explicit Gemini chat/embedding mapping, strict 768, safe error categories và privacy tests | Chưa có extraction/indexing execution, retrieval/grounding, conversation/Guide runtime hoặc AI API; real Gemini chưa gọi | 2026-09-22 |
...
| Tích hợp hệ thống | A | Core Docker runtime smoke + M1/M2/M3 Testcontainers + M4 deterministic provider boundary: PostgreSQL 17.11 pgvector, V1–V4 fresh/upgrade, vector SQL isolation, durable job claim/lease, document/retention races, explicit Spring AI/Google GenAI config, full regression 290/290; mail/provider real network không dùng | Gmail thật manual smoke và real Gemini connectivity chưa chạy | 2026-09-22 |
| Kiểm thử | A | M1 pgvector compatibility 1/1, M2 targeted 36/36, M3 targeted 27/27, M4 provider targeted 22/22, full `mvn -B -ntp clean verify` 290/290; Core golden journeys vẫn giữ pass | Docker smoke chưa script hóa (đã ghi command trong DEVELOPMENT.md) | 2026-09-22 |
| Thiết lập development | A | `docker compose` config + Testcontainers pgvector 0.8.6/PostgreSQL 17.11 verified; V4 fresh/upgrade, M3 lifecycle, M4 provider contract commands, reset/start commands và generated DB schema tài liệu hóa; AI disabled mặc định | Full real-provider/AI indexing Compose runtime chưa verify; Gmail thật chưa verify | 2026-09-22 |
| Reliability | A | Core golden journeys trong Docker runtime (M14/M15), M3 durable job/document/retention races và M4 provider error/timeout/privacy boundary đã pass; lease-token/stale recovery, delete no-resurrection, rejoin cancel, bounded scheduler và safe categories được ghi trong RELIABILITY | Health/metrics endpoint chuyên dụng chưa có (không yêu cầu Core v1); provider/runtime chat monitoring là scope sau | 2026-09-22 |
| Deployment và rollback | A | M14/M15 Dockerfile + Compose/runtime verification; M1 healthcheck topology, M2 additive V4 policy, M4 disabled-by-default provider configuration/request-timeout và Compose config pass | Real Gemini deployment/connectivity chưa verify; connect-timeout SDK limitation tracked; chưa có production deployment target | 2026-09-22 |

...
| AI Chatbot / RAG | B+ | M0–M9 evidence: strict RAG/private API/retention plus immutable two-source Guide corpus, JAR/Docker packaging parity, SQL-bound Guide-only retrieval proven against rogue perfect vector, null-threshold fail-closed grounding and stateless API-AI-010; M9 final full gate 356/356 | Cao ở application/REST boundary, corpus isolation và source docs | Usage guard vẫn M10; real Gemini smoke absent; M5 worker lease heartbeat còn là debt | 2026-09-25 |
...
| API contract | A | M9 runtime OpenAPI exact 38 paths/57 operations, 15 tags, 20 contract tests including Guide method/request/response schemas, bearer/errors/sensitive-field scan; generated Markdown synchronized | Markdown snapshot sync thủ công; chưa có snapshot/breaking-change CI diff (optional theo SD-11) | 2026-09-25 |
| Tích hợp hệ thống | A | Core Docker runtime smoke + M1–M7 Testcontainers/fakes: PostgreSQL 17.11 pgvector, V1–V4, index/RAG/conversation lifecycle, JWT privacy, in-flight revoke and full regression 346/346; no public provider network | Gmail thật manual smoke và real Gemini connectivity/Compose AI journey chưa chạy | 2026-09-24 |
| Kiểm thử | A | M1–M6 targeted suites plus M7 targeted 36/36 and full `mvn -B -ntp clean verify` 346/346; real PostgreSQL/JWT/concurrency/OpenAPI cases; Core regression retained | Docker AI golden journey thuộc M11; Docker smoke chưa script hóa | 2026-09-24 |
...
| Reliability | A | Core golden journeys trong Docker runtime (M14/M15), M3 durable job/document/retention races, M5 bounded index lifecycle and M7 short-TX conversation/provider failure/revoke/delete races pass on PostgreSQL; full 346/346 | No lease heartbeat during long M5 work; abrupt process death can leave chat PROCESSING until recovery policy is approved; runtime AI monitoring is later scope | 2026-09-24 |

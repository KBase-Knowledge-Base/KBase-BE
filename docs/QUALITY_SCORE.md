...
| AI Chatbot / RAG | B | M0–M5 executable evidence: typed disabled-by-default config, KBase-owned ports, deterministic fakes, Flyway V4/pgvector/HNSW, SQL project isolation, durable jobs, explicit Gemini/Spring AI adapters, strict 768 vectors, Tika allowlist extraction, deterministic `kbase-lex-v1`/`chunk-v1`, durable staging/activation, bounded retries, safe errors and delete/stale-lease races; full M5 gate 307/307 | Cao ở mức persistence/async/provider/extraction/indexing boundary và source docs; retrieval/application behavior chưa mở | Semantic retrieval/grounding/citations, conversation/Guide runtime và AI API chưa có; real Gemini smoke intentionally absent; M5 worker lease heartbeat còn là debt | 2026-09-23 |
...
|---|---|---|---|---|
| Backend | A | M0–M16 Core implementation/maintenance + M1–M5 AI foundation/lifecycle/provider/extraction/indexing verified; Core v1 frozen; full M5 regression 307/307 | Core ổn định; retrieval/grounding/application API là phase tiếp theo và chưa thay đổi Core API | 2026-09-23 |
| AI / RAG | B | M0 decision log + M1/M2 executable evidence + M3 job/lifecycle + M4 adapter + M5 extraction/indexing evidence: typed config, vendor-neutral ports, pgvector V4 schema, SQL isolation, durable lease/retry/dedup, explicit Gemini mapping, strict 768, Tika allowlist, deterministic chunking, staging/atomic activation, safe errors and privacy tests | Chưa có semantic retrieval/grounding/citation execution, conversation/Guide runtime hoặc AI API; real Gemini chưa gọi | 2026-09-23 |
...
| Tích hợp hệ thống | A | Core Docker runtime smoke + M1/M2/M3 Testcontainers + M4/M5 deterministic boundaries: PostgreSQL 17.11 pgvector, V1–V4 fresh/upgrade, vector SQL isolation, durable job claim/lease, document/retention/index activation/delete races, explicit Spring AI/Google GenAI config, Tika extraction and full regression 307/307; mail/provider real network không dùng | Gmail thật manual smoke và real Gemini connectivity/Compose indexing chưa chạy | 2026-09-23 |
| Kiểm thử | A | M1 pgvector compatibility 1/1, M2 targeted 36/36, M3 targeted 27/27, M4 provider targeted 22/22, M5 targeted 17/17, full `mvn -B -ntp clean verify` 307/307; Core golden journeys vẫn giữ pass | Docker smoke chưa script hóa (đã ghi command trong DEVELOPMENT.md) | 2026-09-23 |

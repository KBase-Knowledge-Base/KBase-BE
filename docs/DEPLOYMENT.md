...
M7 thêm private Project Assistant và document index REST endpoints; M8 thêm provider-independent retention purge; M9 thêm stateless Guide endpoint và packaged Guide corpus without a new migration, service, volume, secret family or broker. Flyway remains V1–V4. `kbase.ai.enabled=false` keeps Core startup independent of a real Gemini key; when AI is enabled outside tests, provider configuration needs environment credentials. The M9 automated gate uses deterministic fakes and does not verify public Gemini connectivity. Usage guard remains M10 scope.

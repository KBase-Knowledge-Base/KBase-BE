...
- M11 runtime repair adds a packaged KBase port adapter for deterministic Docker verification. It requires `kbase.ai.enabled=true`, explicit `kbase.ai.provider.mode=deterministic`, the `runtime-test` profile and an acknowledgement; Gemini remains the default. The isolated Compose override pins the approved pgvector image and mail double, while the failure companion returns only safe provider unavailability. It has verified AI-enabled startup, Project Assistant/Guide HTTP journeys and Core isolation. *(M11-repair-era note: the security/retention/recovery matrix that was still active work at that point has since been completed — see the M11 freeze section below.)*

### AI v1 M11 runtime verification (freeze, 2026-09-26)

- M11 security/retention/restart/audit matrix đã chạy end-to-end trên isolated Compose project `kbase-m11fix` (alternate host ports, pinned pgvector, MinIO, Redis, Mailpit, deterministic runtime-test provider): security 37/37, retention schedule/restore/purge/race, restart resume/stale-reclaim/persistence/ephemerality, abrupt chat death classification với verified creator-DELETE workaround, log audit 0 hits, consistency audits pass; final gate 377/377.
- In-flight revoke→rejoin race được orchestrate bằng bounded deterministic chat delay (verification-only override): request cũ invalid (403 `PROJECT_ACCESS_FORBIDDEN`, assistant FAILED, 0 citations) dù membership trở lại; request mới dưới replacement membership thành công — continuity guard theo membership identity đã verify runtime.
- AI v1 backend FROZEN 2026-09-26; không đổi integration topology, không real Gemini network.
